...
Capped at `DISAMBIGUATION_IMAGE_LIMIT = 8`. If there are more, only the first 8 go into the image-based disambiguation call — the long tail is truncated. If this becomes a problem, widen the treatment filter or add finer pre-filters (set code, year).
