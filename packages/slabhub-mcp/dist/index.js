#!/usr/bin/env node
import { Condition, GradeProvider, InventoryStage, ItemType, PrismaClient, ProductType, SealedIntegrity, } from '@prisma/client';
import { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js';
import { StdioServerTransport } from '@modelcontextprotocol/sdk/server/stdio.js';
import { z } from 'zod';
import { loadEnv } from './env.js';
import { resolveSessionContext } from './auth.js';
import { SlabhubInventoryService, createInventoryItemSchema, getItemDetailsSchema, listActiveListingsSchema, searchInventorySchema, updateItemStatusSchema, } from './inventory.js';
...
async function main() {
    ...
    try {
        ...
        server.tool('search_inventory', 'Search the authenticated user inventory by item name, set, grade or workflow state.', {
        ...
        server.tool('get_item_details', 'Return a full normalized inventory item record, linked product pricing context, and recent history.', {
            itemId: z.string().trim().min(1),
        }, async (input) => jsonContent(await inventory.getItemDetails(auth.userId, getItemDetailsSchema.parse(input))));
        server.tool('update_item_status', 'Update stage-driven item state, align workflow status, and optionally persist listing or sold pricing.', {
            itemId: z.string().trim().min(1),
        ...
        server.tool('get_analytics_summary', 'Return dashboard-style seller analytics for the authenticated user over the requested day window.', {
            days: z.number().int().min(1).max(365).default(7),
        }, async (input) => jsonContent(await analytics.getAnalyticsSummary(auth.userId, analyticsSummarySchema.parse(input).days)));
        server.tool('list_active_listings', 'List LISTED inventory items for the authenticated user, ordered like the public shop listing view.', {
            limit: z.number().int().min(1).max(100).default(50),
            statusSystemId: z.string().trim().min(1).optional(),
        }, async (input) => jsonContent(await inventory.listActiveListings(auth.userId, listActiveListingsSchema.parse(input))));
        server.tool('create_inventory_item', 'Quick add a new inventory item for the authenticated user, using the same validation rules as the API.', {
