import { Condition, GradeProvider, InventoryStage, ItemType, ProductType, SealedIntegrity, } from '@prisma/client';
import { z } from 'zod';
...
export class SlabhubInventoryService {
    ...
    async searchInventory(userId, input) {
        const where = {
            userId,
        };
        if (input.stage) {
    ...
    async getItemDetails(userId, input) {
        ...
        const linkedProduct = item.refPriceChartingProduct;
        return {
            ...
            linkedProduct: linkedProduct
                ? {
                    ...
                    set: linkedProduct.set
                        ...
                        : null,
                    recentSales: linkedProduct.sales.map((sale) => ({
                        id: sale.id,
                        title: sale.title,
    ...
    async updateItemStatus(userId, input) {
        ...
        const soldDate = input.soldDate !== undefined
        ...
        const statusChanged = existing.statusId !== updated.statusId;
        ...
        if (statusChanged || stageChanged) {
            await this.recordHistory(userId, input.itemId, {
                ...
                fromStatusId: existing.statusId ?? undefined,
                toStatusId: updated.statusId ?? undefined,
            });
        }
        return this.transformItem(updated);
    }
    async listActiveListings(userId, input) {
        const where = {
            userId,
    ...
    async createInventoryItem(auth, input) {
        ...
        if (created.refPriceChartingProductId) {
            ...
            if (marketPrice !== null) {
                ...
                created.marketPriceSnapshot = marketPrice;
            }
        }
    ...
    transformItem(item) {
        ...
        if (item.cardVariant?.card) {
            ...
            cardProfile = {
                ...
                imageUrl: Array.isArray(item.photos) && item.photos.length > 0
    ...
    getMarketPrice(item) {
        ...
        if (item.itemType === ItemType.SINGLE_CARD_GRADED) {
            ...
            const gradedSales = sales.filter((sale) => String(sale.grade ?? '').includes(gradeValue));
        ...
        if (item.itemType === ItemType.SINGLE_CARD_RAW) {
            const rawSales = sales.filter((sale) => {
    ...
    validateCreateInput(input) {
        if (input.itemType === ItemType.SINGLE_CARD_RAW) {
            if (!input.cardVariantId && !input.refPriceChartingProductId && !input.productName) {
                throw new Error('cardVariantId, refPriceChartingProductId, or productName is required for SINGLE_CARD_RAW.');
            }
            return;
        }
        if (input.itemType === ItemType.SINGLE_CARD_GRADED) {
            if (!input.cardVariantId && !input.refPriceChartingProductId && !input.productName) {
                throw new Error('cardVariantId, refPriceChartingProductId, or productName is required for SINGLE_CARD_GRADED.');
            }
    ...
    async resolveTargetStatus(userId, input) {
        let selectedById = null;
        let selectedBySystemId = null;
        if (input.statusId) {
    ...
    async recordHistory(userId, itemId, input) {
