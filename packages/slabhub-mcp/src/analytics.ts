import { PrismaClient, ShopEventType } from '@prisma/client';
...
export class SlabhubAnalyticsService {
  ...
  async getAnalyticsSummary(userId: string, days = 7) {
    ...
    for (let index = 0; index < days; index += 1) {
      ...
      viewsByDay.set(day, { date: day, views: 0, unique: new Set<string>() });
    }
    ...
    const topItemsMap = new Map<string, { name: string; views: number }>();
    for (const event of events as any[]) {
    ...
    for (const event of events as any[]) {
      ...
      if (event.channel) {
        ...
        try {
          ...
          const isInternal =
            internalDomains.some((domain) => hostname === domain || hostname.endsWith(`.${domain}`)) ||
    ...
    const countriesMap = (events as any[])
    ...
    const totalViews = (events as any[]).filter((event) => event.type === ShopEventType.VIEW_SHOP).length;
    ...
    const inquiries = (events as any[]).filter((event) => event.type === ShopEventType.INQUIRY_START).length;
